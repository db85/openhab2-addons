...
public class ParrotFlowerDiscoveryService extends AbstractDiscoveryService {
    ...
    private Completable loadData(ParrotFlowerBridgeHandler bridge) {
        ...
        return bridge.loadAuth()
                .andThen(Completable.mergeArray(parrotFlowerPowerApi.loadProfile().doOnSuccess((profileResponse) -> {
                    UserProfile userProfile = profileResponse.getUserProfile();
                    ThingUID localWeatherThing = new ThingUID(THING_TYPE_USER_PROFILE,
                            userProfile.getEmail().toLowerCase().replaceAll("[^a-zA-Z0-9_]", ""));
                    Map<String, Object> properties = new HashMap<>();
                    thingDiscovered(DiscoveryResultBuilder.create(localWeatherThing).withLabel("User Profile")
                            .withRepresentationProperty("--tt--").withBridge(bridge.getThing().getBridgeUID())
